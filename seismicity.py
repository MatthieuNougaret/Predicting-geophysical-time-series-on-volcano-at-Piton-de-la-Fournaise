...
def split_data(df, date_time, OUTPUT_PATH, generate_figure=True):
    ...
    split_infos['train_times'] = str((str(date_time[0]), str(date_time[cut_train])))
    split_infos['valid_times'] = str((str(date_time[cut_train]), str(date_time[cut_valid])))
    split_infos['test_times'] = str((str(date_time[cut_valid]), str(date_time[len(df)-1])))
    ...
    if generate_figure:
        ...
        plt.ylabel("Hourly seismicity", fontsize=16)
...
def build_dense_model(args, num_features):
    return tf.keras.Sequential([
        #tf.keras.layers.Lambda(lambda x: x[:, -1:, :]),
        tf.keras.layers.Flatten(),
        tf.keras.layers.Dense(192, activation='swish', kernel_initializer='glorot_uniform'),
        tf.keras.layers.Dense(args['OUT_STEPS'] * num_features, kernel_initializer='glorot_uniform'),
        tf.keras.layers.Reshape([args['OUT_STEPS'], num_features])
    ])
...
if __name__ == "__main__":
    ...
    shutil.copyfile(ROOT_PATH / 'seismicity.ini', DEST_PATH / 'seismicity.ini')

    if config['METADATA'].getboolean('SAVE_PY_COPY'):
        shutil.copyfile(ROOT_PATH / 'seismicity.py', DEST_PATH / 'seismicity.py')
